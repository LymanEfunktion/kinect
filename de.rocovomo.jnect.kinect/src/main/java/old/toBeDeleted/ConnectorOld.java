...
public class ConnectorOld extends Observable {

	private KinectManager kinect;
	private Body model;
	private GestureProxy proxy;

	private static Logger logger = Logger.getLogger(ConnectorOld.class);
	...
	public void connectAction(ActionProvider provider) {
		SpeechDetector action = provider.getProvided();
		if (action.isGestureEnabled()) {
			try {
				...
				logger.info("GestureListener added " + provider.getProvided()
						+ ":" + provider.getProperties().get(SpeechDetector.TYPE));
